...
export default function ProjectsSection() {
    ...
    const [selectedCategory, setSelectedCategory] = useState(ProjectCategory.ALL);
    const [selectedId, setSelectedId] = useState<string | null>(null);
    const [selectedTag] = useState("All Tags");
    const [selectedProject, setSelectedProject] = useState<Project | null>(null);
    const [isModalOpen, setIsModalOpen] = useState(false);
        ...
        <section id="projects" className="py-20 scroll-mt-20">
            <div className="mb-12 text-center">
                ...
                <div className="flex flex-wrap gap-3 justify-center mt-8">
                    {isLoading ? (
                        Array.from({ length: 4 }).map((_, i) => (
                            <div key={`cat-skeleton-${i}`} className="w-24 h-10 rounded-xl bg-gray-200 dark:bg-white/5 animate-pulse" />
                        ))
                    ) : (
                        categories.map((cat) => {
                            const isActive = selectedCategory === cat;
            ...
            {/* تم إلغاء الـ motion والـ AnimatePresence هنا لضمان ظهور المشاريع فورا بدون أنميشن */}
